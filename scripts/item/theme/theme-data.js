...
export class ThemeData extends foundry.abstract.TypeDataModel {
	static defineSchema() {
		...
		return {
			...
			specialImprovements: new fields.ArrayField(
				new fields.EmbeddedDataField(abstract.SpecialImprovementData),
				{
					initial: () =>
						Array(5)
							.fill()
					...
					validate: (improvements) => improvements.length === 5,
				},
			),
	...
	static migrateData(source) {
		let powerTagsArray = source.powerTags;
		if (source.powerTags != null && !Array.isArray(powerTagsArray))
			powerTagsArray = Object.values(source.powerTags);

		const numPowerTags = powerTagsArray?.length;
		if (numPowerTags < 10) {
			source.powerTags = [
				...(powerTagsArray ?? []),
		...
		if (numPowerTags > 10) {
			source.powerTags = powerTagsArray.slice(0, 10);
		}

		let weaknessTagsArray = source.weaknessTags;
		if (source.weaknessTags != null && !Array.isArray(weaknessTagsArray))
			weaknessTagsArray = Object.values(source.weaknessTags);

		const numWeaknessTags = weaknessTagsArray?.length;
		...
		if (numWeaknessTags > 2) {
			source.weaknessTags = weaknessTagsArray.slice(0, 2);
		}
